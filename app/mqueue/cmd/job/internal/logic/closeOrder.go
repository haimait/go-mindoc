...
func (l *CloseHomestayOrderHandler) ProcessTask(ctx context.Context, t *asynq.Task) error {
	...
	if err := json.Unmarshal(t.Payload(), &p); err != nil {
		...
		return errors.Wrapf(errors.New("close order fail"), "closeHomestayOrderStateMqHandler payload err:%v, payLoad:%+v", err, t.Payload())
	}
	fmt.Println("CloseHomestayOrderHandler update order status")
	//
